...
import { paypalAccounts, earliestSearchable, MAX_HISTORY_DAYS } from './paypal-client.mjs';
import { fetchRates, mergeRates } from './fx.mjs';
import { openStore } from './store.mjs';
import { loadLiveConfig } from './config.mjs';
...
export async function runSync({
  ...
  for (const paypal of paypalAccounts(env, { fetchImpl })) {
    ...
    const requestedSince = doFull ? windowStart : Math.max(windowStart, Math.floor(ledger.watermark / 1000) - 45 * 86400);

    // history_months can be set far beyond what PayPal allows (Stripe has no
    // such limit), so PayPal gets its own, separately clamped start date.
    // Using anything older here is exactly what causes a 400 on a full sync.
    const earliest = earliestSearchable(now);
    const since = Math.max(requestedSince, earliest);
    const clamped = requestedSince < earliest;

    const accountReport = { account: key, since, full: doFull, fetched: {}, complete: true };
    if (clamped) {
      accountReport.note = `Only searching the last ${MAX_HISTORY_DAYS} days. PayPal does not allow searching further back than that, even though your other settings go further.`;
    }

    try {
      const res = await paypal.transactions({
        since,
        // PayPal cannot reliably search transactions from the last few hours,
        // so the newest edge of the window stays 3 hours behind "now".
        until: Math.floor(now / 1000) - 3 * 3600,
        deadline,
    ...
    for (const [id, c] of Object.entries(ledger.charges)) {
      if (c.created < earliest - 86400) delete ledger.charges[id];
    }
...
export async function rebuild({ store, config, now = Date.now() } = {}) {
  ...
  const cfg = config || (await loadLiveConfig({ store: s, force: true }));
  const { charges, invoices, sessions, subscriptions } = await readLedger({ store: s });
...
/** Everything currently stored, flattened across accounts. */
export async function readLedger({ store } = {}) {
  const s = store || (await openStore('receipts'));
  const state = (await s.get(STATE)) || { accounts: {} };
  const labels = Object.keys(state.accounts);
  const out = { charges: [], invoices: [], sessions: [], subscriptions: [] };
  for (const label of labels.length ? labels : ['account_1']) {
    const ledger = await s.get(`${LEDGER}_${label}`);
    if (!ledger) continue;
    out.charges.push(...Object.values(ledger.charges || {}));
    out.invoices.push(...(ledger.invoices || []));
    out.sessions.push(...(ledger.sessions || []));
    out.subscriptions.push(...(ledger.subscriptions || []));
  }
  return out;
}
